...
import { Search, Download, Trash2, X, ChevronDown } from 'lucide-react';
import { exportToCSV, formatDate } from '@/utils/adminUtils';

const STATUS_OPTIONS = ['new', 'in_progress', 'responded', 'closed'];
const TYPE_OPTIONS = ['general', 'sales', 'technical', 'support', 'partnership'];
...
export default function AdminEnquiries() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await base44.entities.ContactEnquiry.list('-created_date', 500);
      setItems(data);
    } catch (err) { console.error(err); } finally { setLoading(false); }
  };

  const filtered = items.filter(i => {
    const matchSearch = !search || `${i.name} ${i.email} ${i.company} ${i.message}`.toLowerCase().includes(search.toLowerCase());
    const matchStatus = !statusFilter || i.status === statusFilter;
    return matchSearch && matchStatus;
  });

  const handleExport = () => {
    exportToCSV('contact-enquiries.csv', filtered.map(i => ({
      Name: i.name, Company: i.company, Email: i.email, Phone: i.phone,
      Type: i.inquiry_type, Status: i.status, Message: i.message, Notes: i.notes,
      Date: formatDate(i.created_date),
    })));
  };
  ...
  const updateNotes = async (id, notes) => {
    await base44.entities.ContactEnquiry.update(id, { notes });
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{filtered.length} {filtered.length === 1 ? 'enquiry' : 'enquiries'}</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 text-sm pl-10 pr-4 py-2.5 rounded-md focus:outline-none focus:border-rose-500"
          />
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="px-5 py-8 text-center text-slate-400">No enquiries found.</td></tr>
              ) : filtered.map(item => (
                <tr key={item.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => setSelected(item)}>
                  ...
                  <td className="px-5 py-3.5 hidden md:table-cell">
                    <span className="text-xs text-slate-600 capitalize">{item.inquiry_type || 'general'}</span>
                  </td>
                  <td className="px-5 py-3.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h3 className="font-semibold text-slate-900">Enquiry Detail</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><span className="text-xs text-slate-500 block">Inquiry Type</span><span className="font-medium text-slate-900 capitalize">{selected.inquiry_type || 'general'}</span></div>
              ...
              <div>
                ...
                <p className="text-sm text-slate-700 bg-slate-50 p-4 rounded-md">{selected.message}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-white border border-slate-200 text-sm px-3 py-2.5 rounded-md focus:outline-none focus:border-rose-500 resize-none"
                />
              </div>
            </div>
